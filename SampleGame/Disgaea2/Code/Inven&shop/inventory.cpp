...
void inventory::update(void)
{
	...
	if(_cursorTurnOver && KEYMANAGER->isOnceKeyDown(VK_RIGHT))
	{
		...
		{
			tagItem temp;
			temp = _house->gethouseItemArr(_house->getStartRenderPoint() + (_currentItemNum - _startRenderPoint));
			_house->setPushDownX(_isPushDownX);

			_house->setCurrentItemNum(_house->getStartRenderPoint() + (_currentItemNum - _startRenderPoint));
			_cursorTurnOver = false;
			_house->setCursorTurnOver(true);
		}
	...
	if(_cursorTurnOver && KEYMANAGER->isOnceKeyDown('X'))
	{
		...
		{
			tagItem temp;
			temp = _house->gethouseItemArr(_house->getChoiceItemNum());
			_house->sethouseItemArr(_inven[_currentItemNum], _house->getChoiceItemNum());
			temp.itemRc = _inven[_currentItemNum].itemRc;

			_inven[_currentItemNum] = temp;

			_house->setPushDownX(0);
			_house->setChoiceItem(false);
			_choiceItem = true;
		...
		if(_isPushDownX >= 1 && _choiceItem)
		{
			...
			tagItem temp;
			temp = _inven[_currentItemNum];
			_inven[_currentItemNum] = _inven[_choiceItemNum];
			_inven[_choiceItemNum] = temp;

			temp.itemRc = _inven[_currentItemNum].itemRc;
			_inven[_currentItemNum].itemRc = _inven[_choiceItemNum].itemRc;
			_inven[_choiceItemNum].itemRc = temp.itemRc;
...
void inventory::setItemVector(tagItem it)
{
...
bool inventory::getFullItem(void)
{
	for(int i = 0; i < _totalItemNum; i++)
	{
		if(_inven[i].name == "( IDLE )")
...
void inventory::writeInvenFile(tagItem* arrayItem)
{
...
void inventory::readInvenFile(void)
{
	//맨 첨에 읽기
	HANDLE file;
	DWORD read;
...
void inventory::setInvenItemArr(tagItem temp, int currentNum)
{
	RECT rcTemp;
...
int inventory::progressbarInven(long top, long bottom)
{
