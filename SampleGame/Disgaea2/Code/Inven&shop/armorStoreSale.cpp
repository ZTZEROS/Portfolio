...
HRESULT armorStoreSale::init(void)
{
	_currentItemNum = 0;
	_isPushDownX = 0;
	_totalItemNum = 16;
...
void armorStoreSale::release(void)
{
}

void armorStoreSale::update(int money)
{
	_totalMoney = money;
	...
	if(_isPushDownC) //KEYMANAGER->isOnceKeyDown('C'))
	{
		for(int i = 0; i < _totalItemNum; i++)
		{
			_inStoreinven[i].itemRc = RectMake(50, 140 + ( 50 * _currentItemNum), 300, 50);
		}
...
void armorStoreSale::render(void)
{
	...
	for(int i = 0; i < _totalItemNum; i++)
	{
		...
		{
			//Rectangle(getIMemDC(), _inven[i].itemRc.left, _inven[i].itemRc.top, _inven[i].itemRc.right, _inven[i].itemRc.bottom);
			//_inStoreinven[i].img->render(getIMemDC(), _inStoreinven[i].itemRc.left+30, _inStoreinven[i].itemRc.top);
...
void armorStoreSale::readInStoreInvenFile(void)
{
	//맨 첨에 읽기
	HANDLE file;
	DWORD read;
...
void armorStoreSale::writeInStoreInvenFile(tagItem* arrayItem)
{
...
int armorStoreSale::progressbarSaleInven(long top, long bottom)
{
