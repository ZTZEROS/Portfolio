...
unsigned int IOCP_Server::ReceivePacket(UINT64 sessionKey, SerialPacketQueue<IOCP_PacketHeader>* receivedPacketSPQ_Address)
{
...
unsigned int IOCP_Server::SendPacket(UINT64 sessionKey, SerialPacketQueue<IOCP_PacketHeader>* sendingPacketSPQ_Address)
{
...
bool IOCP_Server::Disconnect(UINT64 sessionKey)
{
	UINT sessionIndex;
...
unsigned int IOCP_Server::PostSend(UINT64 sessionKey)
{
	UINT sessionIndex;
...
unsigned int IOCP_Server::PostDisconnect(UINT64 sessionKey)
{
	UINT sessionIndex;
...
bool IOCP_Server::Start(LPCWSTR ipAddress, UINT port, UINT workThreadCount, BOOL nagleOff, UINT maximumClientCount)
{
