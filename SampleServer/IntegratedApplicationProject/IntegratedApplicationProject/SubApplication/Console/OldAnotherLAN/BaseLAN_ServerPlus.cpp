			...
			FOR(i, TOTAL_ANOTHER_LAN_CLIENT_COUNT)
			{
				if (ALAN_CSV[i].CurrentPhaseIndex == ANOTHER_LAN_CLIENT_PHASE_ACCEPT)
				{
					...
					ELSE
					{
						...
						ALAN_CSV[i].SessionKey = sessionKey;
						...
						ALAN_CSV[i].IO_Count = 1; //not danger?
						...
						THIS->OnClientJoin(ALAN_CSV[i].Address, ALAN_CSV[i].SessionKey);

						PostQueuedCompletionStatus(ALAN_S_COMMON_CP, NULL, ALAN_CSV[i].SessionKey, &ALAN_CSV[i].OverlappedReceive);
...
unsigned int WINAPI BaseLAN_Server::DisconnectAnotherLAN_Client(LPVOID instanceAddress)
{
	...
	while (!ALAN_S_SHUTDOWN)
	{
		...
		if(GetQueuedCompletionStatus(ALAN_S_DISCONNECT_CP, &transferredSize, &completionKey, &overlapped, INFINITE))
		{
			...
			{
				THIS->OnClientLeave(ALAN_CSV[clientIndex].SessionKey);

				closesocket(ALAN_CSV[clientIndex].Socket);
				//shutdown(ALAN_CSV[clientIndex].Socket, SD_BOTH); //셧다운 사용시, 센드 중 디스커넥트 되고 다시 어셉트를 받으면서 같은 소켓을 할당받는 경우 새 접속자에게 이전 접속자의 패킷을 보내는 경우가 생길 수 있을까?
				...
				ALAN_CSV[clientIndex].SessionKey = 0xffffffffffffffff;
...
unsigned int WINAPI BaseLAN_Server::WorkAnotherLAN_Server(LPVOID instanceAddress)
{
	...
	while (!ALAN_S_SHUTDOWN)
	{
		...
		{
			if (overlapped) //impossible case
			{
				sessionKey = completionKey;
				clientIndex = sessionKey >> ANOTHER_LAN_CLIENT_INDEX_SHIFTED_BIT_COUNT;
				if (!(clientIndex < TOTAL_ANOTHER_LAN_CLIENT_COUNT) || ALAN_CSV[clientIndex].SessionKey != sessionKey)
				{
					continue;
				}
				...
				if (&ALAN_CSV[clientIndex].OverlappedReceive == overlapped)
				{
					//rcvPost
					{
						...
						if (0 < transferredSize)
						{
							...
							THIS->OnRecv(ALAN_CSV[clientIndex].SessionKey, transferredSize);
						}
						...
						if (!InterlockedDecrement(&ALAN_CSV[clientIndex].IO_Count))
						{
							...
							if(ALAN_CSV[clientIndex].ReceiveCount == 1 && ALAN_CSV[clientIndex].SendCount == 1)
							{
								...
								PostQueuedCompletionStatus(ALAN_S_DISCONNECT_CP, NULL, ALAN_CSV[clientIndex].SessionKey, &ALAN_CSV[clientIndex].OverlappedReceive);
								//THIS->Disconnect(ALCSV[clientIndex].SessionKey);
							}

							//if (InterlockedExchange(&ALAN_CSV[clientIndex].Connected, 0))
							//{
							//	if (ALAN_CSV[clientIndex].SessionKey == sessionKey && ALAN_CSV[clientIndex].CurrentPhaseIndex != ANOTHER_LAN_CLIENT_PHASE_ACCEPT)
							//	{
							//		ALAN_CSV[clientIndex].CurrentPhaseIndex = ANOTHER_LAN_CLIENT_PHASE_DISCONNECT;
							//
							//		InterlockedIncrement(&RCV_DISCONNECTION_COUNT);
					...
					//sndPost
					{
						if (InterlockedExchange(&ALAN_CSV[clientIndex].Sendable, 0))
						{
							...
							if (0 < transferredSize)
							{
								...
								THIS->OnSend(ALAN_CSV[clientIndex].SessionKey, transferredSize);
							}
							else
							{
								...
								if (result == SOCKET_ERROR)
								{
									...
									else
									{
										...
										printf("[WSASend] clientIndex : %u, sessionKey : %llu, IO_Count : %d, WSAGetLastError() : %d" CRALF, clientIndex, ALAN_CSV[clientIndex].SessionKey, ALAN_CSV[clientIndex].IO_Count, WSAGetLastError());
									}
								}
							}
			...
			if (overlapped)
			{
				...
				if (InterlockedExchange(&ALAN_CSV[clientIndex].Connected, 0))
				{
					if (ALAN_CSV[clientIndex].SessionKey == sessionKey && ALAN_CSV[clientIndex].CurrentPhaseIndex != ANOTHER_LAN_CLIENT_PHASE_ACCEPT)
					{
						...
						PostQueuedCompletionStatus(ALAN_S_DISCONNECT_CP, NULL, ALAN_CSV[clientIndex].SessionKey, overlapped);
						//THIS->Disconnect(ALCSV[clientIndex].SessionKey);
					}
					else InterlockedExchange(&ALAN_CSV[clientIndex].Connected, 1);
				}
