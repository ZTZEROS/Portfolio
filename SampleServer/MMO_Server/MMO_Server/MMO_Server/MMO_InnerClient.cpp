...
MMO_InnerClient::MMO_InnerClient()
{
	ZeroMemory(&ICP, sizeof(ICP));
...
bool MMO_InnerClient::Initialize()
{
	{
		ZeroMemory(&ICP, sizeof(ICP));
...
bool MMO_InnerClient::Update()
{
	...
	{
		if (ICP.SessionKey && ICP.Index) ProcessWork();
	}

	return 0;
}

bool MMO_InnerClient::Render()
{
	//if (TIMER.ShowCounter()) //CheckRender()
	{
		printf(
			"====== MMO_InnerClient =====" CRALF
...
bool MMO_InnerClient::Terminalize()
{
	{
