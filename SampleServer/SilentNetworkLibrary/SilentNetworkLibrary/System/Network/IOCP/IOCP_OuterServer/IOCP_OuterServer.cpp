...
IOCP_OuterServer::IOCP_OuterServer()
{
...
bool IOCP_OuterServer::Initialize()
{
	WSADATA wsaDatum;
	...
	{
		{
			...
			SUB_THREAD_V.resize(IOCP_OUTER_SERVER_SUB_THREAD_COUNT + WorkThreadCount);
	...
	{
		if (SUB_THREAD_V.size() - IOCP_OUTER_SERVER_SUB_THREAD_COUNT);
		else SUB_THREAD_V.resize(IOCP_OUTER_SERVER_SUB_THREAD_COUNT + systemInformation.dwNumberOfProcessors * 2);

		COMMON_CP = CreateIoCompletionPort(INVALID_HANDLE_VALUE, nullptr, NULL, SUB_THREAD_V.size() - IOCP_OUTER_SERVER_SUB_THREAD_COUNT);
...
bool IOCP_OuterServer::Terminalize()
{
...

	{
		...
		FOR(i, SUB_THREAD_V.size() - IOCP_OUTER_SERVER_SUB_THREAD_COUNT) //close work thread
		{
