...
IOCP_InnerServer::IOCP_InnerServer()
{
	IP_Address = L"127.0.0.1";
	Port = 27050;
	MaximumClientCount = 10;
	WorkThreadCount = 2;
}

IOCP_InnerServer::~IOCP_InnerServer() {}

bool IOCP_InnerServer::Initialize()
{
	WSADATA wsaDatum;
	...
	{
		{
			...
			SUB_THREAD_V.resize(IOCP_INNER_SERVER_SUB_THREAD_COUNT + WorkThreadCount);
	...
	{
		if (SUB_THREAD_V.size() - IOCP_INNER_SERVER_SUB_THREAD_COUNT);
		else SUB_THREAD_V.resize(IOCP_INNER_SERVER_SUB_THREAD_COUNT + systemInformation.dwNumberOfProcessors * 2);

		COMMON_CP = CreateIoCompletionPort(INVALID_HANDLE_VALUE, nullptr, NULL, SUB_THREAD_V.size() - IOCP_INNER_SERVER_SUB_THREAD_COUNT);
		...
		SUB_THREAD_V[IOCP_INNER_SERVER_SUB_THREAD_ACCEPT] = subThread;

		subThread.Handle = (HANDLE)_beginthreadex(nullptr, 0, ConnectServer, this, 0, &subThread.IdentificationNumber);
		SUB_THREAD_V[IOCP_INNER_SERVER_SUB_THREAD_CONNECT] = subThread;
		...
		SUB_THREAD_V[IOCP_INNER_SERVER_SUB_THREAD_DISCONNECT] = subThread;

		FOR(i, (SUB_THREAD_V.size() - IOCP_INNER_SERVER_SUB_THREAD_COUNT))
		{
			subThread.Handle = (HANDLE)_beginthreadex(nullptr, 0, WorkServer, this, 0, &subThread.IdentificationNumber);
			SUB_THREAD_V[IOCP_INNER_SERVER_SUB_THREAD_COUNT + i] = subThread;
		}
	}
...
bool IOCP_InnerServer::Terminalize()
{
...

	{
		...
		FOR(i, SUB_THREAD_V.size() - IOCP_INNER_SERVER_SUB_THREAD_COUNT) //close work thread
		{
