...
unsigned int WINAPI IOCP_InnerServer::AcceptClient(LPVOID selfInstanceAddress)
{
	IOCP_InnerServer* thisInstanceAddress;

	thisInstanceAddress = (IOCP_InnerServer*)selfInstanceAddress;
...
unsigned int WINAPI IOCP_InnerServer::ConnectServer(LPVOID selfInstanceAddress)
{
	IOCP_InnerServer* thisInstanceAddress;
...
unsigned int WINAPI IOCP_InnerServer::DisconnectClient(LPVOID selfInstanceAddress)
{
	IOCP_InnerServer* thisInstanceAddress;
...
unsigned int WINAPI IOCP_InnerServer::WorkServer(LPVOID selfInstanceAddress)
{
	IOCP_InnerServer* thisInstanceAddress;
...
unsigned int IOCP_InnerServer::AcceptClient()
{
...
unsigned int IOCP_InnerServer::ConnectServer()
{
	IOCP_ClientSession clientSession;
	//INT addressLength;
	UINT64 sessionKey;
	...
	//addressLength = sizeof(clientSession.Address);
	sessionKey = 0xffffffffffffffff + CSV.size() / 2;
	sessionIndex = 0xffffffff + CSV.size() / 2;
		...
		if (overlapped)
		{
			...
			//ELSE
			{
...

				{
					...
					else sessionIndex = CSV.size() / 2;

					if (CSV[sessionIndex].CurrentPhase == IOCP_CLIENT_PHASE_CONNECT)
					{
...
unsigned int IOCP_InnerServer::DisconnectClient()
{
	DWORD transferredSize;
	ULONG_PTR completionKey;
	LPOVERLAPPED overlapped;
		...
		if (overlapped)
		{
			//if (InterlockedExchange(&CSV[sessionIndex].Connected, 0) && CSV[sessionIndex].Key == sessionKey)
			{
				if (CSV[sessionIndex].CurrentPhase == IOCP_CLIENT_PHASE_DISCONNECT)
				{
					...
					CSV[sessionIndex].ReceiveQ.Clear();
					CSV[sessionIndex].SendQ.Clear();
					CSV[sessionIndex].ReceiveBQ.Clear();
					CSV[sessionIndex].SendBQ.Clear();
...
unsigned int IOCP_InnerServer::WorkServer()
{
	DWORD transferredSize;
	ULONG_PTR completionKey;
	LPOVERLAPPED overlapped;
